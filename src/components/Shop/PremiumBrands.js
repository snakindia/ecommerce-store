...
export default class PremiumBrands extends Component {
    ...
    componentDidMount() {
        axios.get(API_URL + "/get_branded_category_list?fields=name,image,description")
            .then((res)=> {
                                    ...
                                    brands.map((item, idx) => {
                                        return (
                                             <div class="col-sm-4 col-md-4 bha-mb">
                                                <a href="#">
                                                    <div class="view view-first" style={{height: '250px'}}>
                                                    <img class="img-fluid" src={item.image.length > 0 ? item.image : API_IMAGE_PATH + 'default/default.jpg'} alt="Card image" />
